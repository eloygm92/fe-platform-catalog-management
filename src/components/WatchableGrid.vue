...
<script setup lang="ts">
  ...
  const image_url = ref<string>(props.watchable.poster_path ? import.meta.env.VITE_IMG_CDN + 'w185/' + props.watchable.poster_path : import.meta.env.VITE_404_IMG)
  //const toParams = ref<RouteParams>({ path: '/watchable/'+ props.watchable.id, component: WatchableDetail, watchable: props.watchable })
  const toParams = ref<RouteParams>({ name: 'watchableDetail', props: { watchable: watchableData }, params: { id: props.watchable.id } })
